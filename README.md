...
```bash
npm install     # 仓库未包含根目录 package-lock.json，安装时会自动生成
...
### 目录概览
- `miniapp/src/pages/home`：城市选择 + 地图视野加载小区（噪音等级/TopN 筛选、地图刷新/回到中心按钮、Marker 卡片与列表联动）。
- `miniapp/src/pages/compound`：小区地图热力点 + 楼栋列表（等级/TopN 筛选、Marker 卡片、边界 polygon、加载/空态遮罩）。
...
2. 进入 `miniapp`：`cd miniapp && npm install`（首次安装 Taro 依赖可能较慢；**仓库不包含根目录与 miniapp 的 lockfile，安装时会现场生成 package-lock.json**）。
...
- 地图类页面均提供暗色提示条与右上角快捷按钮（回到中心、刷新视野/楼栋），并在加载/错误/空态时给出遮罩提醒；
- Marker、标签与卡片风格统一使用 `miniapp/UIDesign.md` 中的色板与圆角规则，浮层使用毛玻璃+渐变按钮，确保视觉一致；
- 噪音等级的胶囊标签含颜色圆点，列表/卡片点击有轻微上浮与描边，避免“生硬”体验。
- Marker 图标使用在线示例资源（腾讯地图演示地址），仓库不包含二进制图片文件，便于在受限环境推送。

### 推送前二进制检查

- 若远端限制二进制文件，可在根目录执行 `npm run check:binaries`，自动扫描所有 git 已跟踪文件的扩展名和内容；
- 如有提示命中二进制，请改用在线资源或将文件转换为文本格式，再提交推送。
