...
export default function CompoundPage() {
  ...
  async function loadBuildings(id: number) {
    ...
    try {
      ...
      setLoadError(err.message || '加载楼栋失败');
      ...
      <View className='map-wrapper'>
        ...
        <View className='map-hint'>
          ...
          <View className='map-actions'>
            ...
            <View className='action-btn' onClick={() => loadBuildings(compoundId)}>
              重新加载
          </View>
        </View>
        {loading && <View className='map-overlay'>正在加载楼栋视图...</View>}
        {loadError && !loading && <View className='map-overlay error'>{loadError}</View>}
        {!loading && !loadError && filteredBuildings.length === 0 && (
          <View className='map-overlay'>筛选后暂无楼栋，请切换等级或关闭 TopN 推荐</View>
        )}
      </View>
